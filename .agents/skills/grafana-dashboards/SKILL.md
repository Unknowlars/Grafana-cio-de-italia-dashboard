...
description: >
  ...
  schema concepts, rows/tabs/groupings, panels, transformations, variables,
  Prometheus/PromQL, Mimir/Grafana Cloud Metrics, Loki/LogQL,
  Tempo/TraceQL, Alloy/OpenTelemetry pipelines, Elasticsearch/Lucene/ES|QL
  caveats, alerting, provisioning, Git Sync, and validation workflows.
  Use whenever the user asks to create a dashboard, add/refactor panels,
  convert rows to tabs, fix queries, write PromQL/LogQL/SQL/Lucene, configure
  transforms, define variables, review dashboard JSON, improve operator UX,
  create alert rules, or export/provision Grafana resources.
---
...
- Grafana 13 changes: https://grafana.com/docs/grafana/latest/whatsnew/whats-new-in-v13-0/
...
- `references/grafana-v2-tabs.md` — Grafana 13/dynamic dashboard, v2/OaC layout concepts, tabs migration workflow, and validation.
...
## Grafana 13 update notes

When working with Grafana 13+:

- Dynamic dashboards are generally available and on by default.
...
- Rows and tabs are first-class groupings. Dashboards can use rows, tabs, custom grid, auto grid, nesting, repeats, and visibility rules.
- Ad hoc filters were renamed **Filters** in the UI, but the schema still uses `AdhocVariable`.
- Git Sync for dashboards and folders is generally available.
...
- Be careful upgrading early Git Sync/unified-storage users from 12.x to 13.0.0; review Grafana upgrade notes and prefer a fixed later patch release.
